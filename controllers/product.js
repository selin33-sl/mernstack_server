...
export const updateProduct = asyncError(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    messgae: "Product Update Successfully",
  });
});
...
export const addProductImage = asyncError(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    messgae: "Image Added Successfully",
  });
});
...
export const deleteProductImage = asyncError(async (req, res, next) => {
  ...
  product.images.forEach((item, index) => {
    if (item.public_id.toString() === id.toString()) isExist = index;
  });
  ...
  res.status(200).json({
    success: true,
    messgae: "Image Deleted Successfully",
  });
});
...
export const deleteProduct = asyncError(async (req, res, next) => {
  ...
  //Videoda buradaki kod şu şekildeydi
  //ama remove fonksiyonu kullanımdan kalktı sanırım.
